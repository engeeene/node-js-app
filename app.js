...
async function generate(amount) {
	...
	CREATE TABLE IF NOT EXISTS motherboard (
		...
		north_bridge boolean NOT NULL,
		south_bridge boolean NOT NULL,
		socket sockets NOT NULL,
		memory_type memory_types NOT NULL,
		sata smallint NOT NULL CHECK (sata > 0),
		max_cooler_weigth integer NOT NULL CHECK (max_cooler_weigth > 0)
	);
	CREATE TABLE IF NOT EXISTS cpu (
		...
		required_power integer NOT NULL CHECK (required_power > 0),
		socket sockets NOT NULL,
		core_frequency numeric NOT NULL CHECK (core_frequency > 0),
		cores smallint NOT NULL CHECK (cores > 0),
		min_cooler_efficiency efficiency_type NOT NULL,
		chip chips NOT NULL
	);
	CREATE TABLE IF NOT EXISTS cpu_cooler (
		...
		price numeric NOT NULL CHECK (price > 0),
		required_power integer NOT NULL CHECK (required_power > 0),
		weigth integer NOT NULL CHECK (weigth > 0),
		silent boolean NOT NULL,
		efficiency efficiency_type NOT NULL
	);
	CREATE TABLE IF NOT EXISTS motherboard_cpu_cooler (
		id serial PRIMARY KEY,
		socket sockets NOT NULL,
		id_cpu_cooler integer REFERENCES cpu_cooler (id)
	);
	CREATE TABLE IF NOT EXISTS gpu (
		...
		price numeric NOT NULL CHECK (price > 0),
		required_power integer NOT NULL CHECK (required_power > 0),
		core_frequency integer NOT NULL CHECK (core_frequency > 0),
		memory_amount integer NOT NULL CHECK (memory_amount > 0),
		memory_frequency integer NOT NULL CHECK (memory_frequency > 0),
		additional_pin boolean NOT NULL,
		chip chips NOT NULL
	);
